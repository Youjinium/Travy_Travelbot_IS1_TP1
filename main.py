...
pattern_matching = [
    ...
    (r'I have a (low|middle|high) budget in mind[.]?', ['I see that you have a {0} budget. Staying {1} is a great choice in that case. Your reservation has been confirmed.', 'Hmm, looks like your budget is a bit out of the ordinary. Perhaps you could try again with a low, middle, or high budget?'], 'accomodation'),
...
def chatbot_response(entity, is_true, responses, value):
    ...
    elif entity == 'accomodation':
        if is_true:
            accomodation = budget_map.get(value),
            response = responses[0].format(value, accomodation)
        else:
            response = responses[1].format(value)
    else:
        if is_true:
